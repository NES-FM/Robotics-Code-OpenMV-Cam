...
def init_sensor():
	...
	sensor.set_windowing((X_OFFSET_WINDOWING, 0, 240, 240))
...
def black_white_handler():
	...
	if CORNER_ENABLED:
		...
		for b in img.find_blobs([(0, 34)], roi=(0, 15, 240, 215), merge=True, margin=5):
			(x, y, w, h) = b.rect()
			if b.pixels() > 500:
				conf = ((1-b.roundness()) * 0.33) + (b.elongation() * 0.34) + ((180-b.rotation_deg()) * 0.0018333333)
				if conf > 0.8:
	...
	if BALLS_ENABLED:
		...
		for i, detection_list in enumerate(net.detect(img, thresholds=[(math.ceil(MIN_TF_CONF * 255), 255)])):
			...
			for d in detection_list:
				if not do_boxes_overlap(d.rect(), (0, 208, 177, 33)) and not do_boxes_overlap(d.rect(), (0, 170, 56, 70)):
					if i == black_ball_id:
		...
		for ball in balls:
			...
			if histogram_classification == classified_as:
				certain = True
			conf = (0.7 * v) + (0.3 * (len(ball.circles_detected)==1))
